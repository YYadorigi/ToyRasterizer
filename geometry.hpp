...
struct Vec2
{
    ...
    inline T norm() const { return std::sqrt(u * u + v * v); }

    friend std::ostream &operator<<(std::ostream &out, const Vec2<T, F> &V)
    {
        out << "(" << V.u << ", " << V.v << ")";
        return out;
    }
...
struct Vec3
{
    ...
    inline Vec3<T, F> &normalized()
    {
        *this = *this * (1. / norm());
        return *this;
    }

    friend std::ostream &operator<<(std::ostream &out, const Vec3<T, F> &v)
    {
        out << "(" << v.x << ", " << v.y << ", " << v.z << ")";
        return out;
    }
